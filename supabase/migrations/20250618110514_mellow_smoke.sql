...
-- Create RLS policies
CREATE POLICY IF NOT EXISTS "Anyone can read places"
  ON places
  FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can add places"
  ON places
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY IF NOT EXISTS "Users can update own places"
  ON places
  FOR UPDATE
  TO authenticated
  USING (
